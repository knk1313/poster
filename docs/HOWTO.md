...
手動で実行
gcloud scheduler jobs run post-news-from-sheet --location=asia-northeast1
...
やること
Xで実際にポスト
fetchYahooNews 側にスケジューラを設定

OpenAI/X エラー時のリトライや rate limit 対策（再実行かスキップの方針決め）
ログと監視（重複や 401/429/5xx を見やすくする）
環境変数管理（prod/stg でキーを切り替えられるように）
